...
export const arrayToWtc = (array) => {
	const wtcFuncs = [];
	const wtcFuncsNames = [];
	let tempCounter = 0;

	array.map((item) => {
		const { table, type, sharedkeys, sharedValues, distencKeysAndValues } =
			item;
		...
		/*
			...
			// 		...items,
			// 	].join(',')}) RETURNING ${
			// 		returning && returning.length !== 0 ? returning.join(',') : '*'
			// 	}
			// 		)
			// 	`;
