					...
					$where: (obj, string = '') => {
						...
						keys.forEach((key, index) => {
							if (clauses[key]) {
								...
								tempString += ` ${key}=(${putInQuotationMark(obj[key])})`;
							}
						});
					...
					$in: (obj, string = '') => {
						...
						keys.forEach((key, index) => {
							if (clauses[key]) {
								...
								tempString += ` ${key} IN (${obj[key].map((item) =>
									putInQuotationMark(item)
								)})`;
							}
						});
	...
	array.map((item) => {
		...
		if (type === 'insert') {
			...
			values.forEach((values) => {
				...
				const func = `${funcName} AS (
					...
					...sharedValues.map((item) => putInQuotationMark(item)),
					...values.map((item) => putInQuotationMark(item)),
				].join(',')}) RETURNING ${
      ...
      `;
			...
			const func = `${funcName} AS (
				...
				(key, index) => `${key}=(${putInQuotationMark(keysAndValues[key])})`
